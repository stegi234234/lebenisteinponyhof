...
<template>
  <div>
      ...
      <div class="container mx-auto px-4">
        ...
        >
          <div class="flex-shrink-0">
            <i
              class="fas fa-info-circle text-amber-600 dark:text-amber-400 text-2xl sm:text-3xl"
            ></i>
          </div>
          <div>
            ...
            <p class="text-amber-800 dark:text-amber-200 text-sm md:text-base leading-relaxed">
              <strong>Technische Störung:</strong> Momentan kommt es leider zu Problemen auf unserer
              Website – insbesondere auf Smartphones. Bitte sende deine Anfrage vorübergehend an
              <a
                href="mailto:sarah@lebenisteinponyhof.de"
                class="underline font-medium hover:text-amber-700 dark:hover:text-amber-100"
                >sarah@lebenisteinponyhof.de</a
    ...
    >
      <div
        class="absolute top-0 right-0 w-64 h-64 bg-primary-color/5 dark:bg-primary-light/5 rounded-full -translate-y-1/3 translate-x-1/3 blur-2xl -z-0"
      ></div>
      <div
        class="absolute bottom-0 left-0 w-80 h-80 bg-primary-color/5 dark:bg-primary-light/5 rounded-full translate-y-1/3 -translate-x-1/3 blur-2xl -z-0"
      ></div>

      <div class="container mx-auto px-4 text-center relative z-10">
        <div class="section-header mb-12 md:mb-16">
          <span
            class="uppercase tracking-wider text-primary-color dark:text-primary-light text-sm font-semibold"
          >
            Entwicklungsförderung mit Pferden
          </span>
          <h2
            id="why-therapy-heading"
            class="text-3xl md:text-4xl font-bold mt-2 mb-4 text-gray-800 dark:text-white"
          >
          ...
          <p class="max-w-2xl mx-auto text-gray-600 dark:text-gray-400 text-lg leading-relaxed">
            ...
            Weise positiv beeinflusst und in ihrer Entwicklung unterstützt – direkt hier auf unserem
            Ponyhof in
        ...
        <div class="mt-12 md:mt-16">
          <AppButton
            :to="{ path: '/blog/foerderung-kinder-ponyreiten-renningen' }"
            variant="primary"
            size="lg"
            class="px-8 py-4 text-lg rounded-xl"
          >
    ...
    <section
      class="testimonials py-16 md:py-20 bg-gray-50 dark:bg-gray-900/30"
      ...
      <div class="container mx-auto px-4 text-center">
        <div class="section-header mb-12">
          ...
          <p class="max-w-3xl mx-auto text-gray-600 dark:text-gray-400 text-lg">
            Lesen Sie, was Familien über ihre Erlebnisse beim Ponyreiten und den Angeboten auf
            unserem Hof in
            {{ businessAddress.city }} berichten.
          </p>
        </div>
    ...
    <section
      class="main-cta-section py-20 md:py-28 relative overflow-hidden"
      aria-labelledby="main-cta-heading"
    >
      <div
        class="absolute inset-0 bg-gradient-to-br from-pink-500 via-primary-color to-orange-400 z-0"
      ></div>
      <div
        class="absolute inset-0 opacity-10 bg-[url('/images/pattern-horses.webp')] bg-repeat z-0"
      ></div>
      <div class="container mx-auto px-4 relative z-10 text-center">
        <h2
          id="main-cta-heading"
          class="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 text-white drop-shadow-lg max-w-4xl mx-auto leading-tight"
        >
          Das Leben ist ein Ponyhof – Erlebe es mit uns in {{ businessAddress.city }}!
        </h2>
        <p
          class="max-w-2xl mx-auto mb-10 text-white text-opacity-95 text-lg md:text-xl leading-relaxed"
        >
          Wir freuen uns darauf, dich und deine Familie auf unserem Ponyhof in
          {{ businessAddress.city }} willkommen zu heißen und euch die wunderbare Welt unserer Ponys
          näherzubringen.
        </p>
        <div class="flex flex-col sm:flex-row justify-center gap-4">
          ...
          <AppButton
            :to="{
              path: '/contact',
              query: { subject: `Anfrage Ponyerlebnis ${businessAddress.city}` },
            }"
            size="xl"
...
<script setup lang="ts">
    ...
    setup(props) {
      return () =>
        h(
          ...
          {
            class:
              'feature-card flex flex-col items-center p-6 bg-white dark:bg-gray-700 rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300 border border-gray-100 dark:border-gray-600/20',
          },
          [
            h(
              'div',
              {
                class: `w-16 h-16 rounded-full ${props.bgColor || 'bg-primary-color/10 dark:bg-primary-light/10'} flex items-center justify-center mb-4`,
              },
              [
                h('i', {
                  class: `${props.icon} text-3xl ${props.iconColor || 'text-primary-color dark:text-primary-light'}`,
                }),
              ]
            ),
            h(
              'h3',
              { class: 'text-xl font-semibold mb-3 text-gray-800 dark:text-white text-center' },
              props.title
            ),
            h(
              'p',
              {
                class:
                  'text-gray-600 dark:text-gray-400 leading-relaxed flex-grow text-sm text-center',
              },
              props.description
            ),
          ]
  ...
  const TestimonialCard = defineComponent({
    ...
    setup(props) {
      const starElements = Array.from({ length: props.stars || 0 }).map(() =>
      ...
      return () =>
        h(
          ...
          {
            class:
              'testimonial-card w-full p-6 bg-white dark:bg-gray-700 rounded-xl shadow-lg border border-gray-100 dark:border-gray-600/20 text-center',
          },
          [
            ...
            h(
              ...
              {
                class:
                  'text-gray-600 dark:text-gray-300 italic mb-4 text-sm leading-relaxed min-h-[6em]',
              },
              `"${props.quote}"`
            ),
            h('div', { class: 'flex flex-col items-center' }, [
              ...
              props.location &&
                h('p', { class: 'text-xs text-gray-500 dark:text-gray-400' }, props.location),
            ]),
          ]
        )
    },
